...
The certificates generated by `scripts/generate-certs.sh` include Subject Alternative Names (SANs) to support multiple hostname formats. Each Galera node certificate includes:

- **CN (Common Name)**: The `.local` alias (e.g., `galera1.local`)
- **DNS.1**: The short hostname (e.g., `galera1`)
- **DNS.2**: The `.local` alias (e.g., `galera1.local`)

This allows the certificates to validate connections using either the short hostname or the `.local` alias. In this lab, we use `.local` aliases in `wsrep_cluster_address` (defined in `docker-compose.yaml`) to match what's advertised in `wsrep_node_address`, but the certificates work with both formats.

For production deployments, include all hostname formats you'll use in the SANs:
- Short hostnames (e.g., `galera1`)
- FQDNs (e.g., `galera1.example.com`)
- Any aliases or CNAMEs that will be used to connect

This flexibility is important because Galera nodes may be referenced by different names in different contexts (e.g., `wsrep_cluster_address` might use FQDNs while `wsrep_node_address` uses IPs or short names).
...
- **Container health overview**
  ...
  galera_passbolt   passbolt/passbolt:5.7.2-1-ce   "/bin/bash -ceu /doc…"   Up 6 minutes (healthy)
